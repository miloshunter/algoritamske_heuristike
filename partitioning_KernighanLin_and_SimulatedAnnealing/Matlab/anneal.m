function [minimum,fval] = anneal(P1, P2, G, loss)
...
% main settings
newsol = @(P)new_sol(P);      % neighborhood space function
Tinit = def.InitTemp;        % initial temp
minT = def.StopTemp;         % stopping temp
cool = def.CoolSched;        % annealing schedule
minF = def.StopVal;
max_consec_rejections = def.MaxConsRej;
max_try = def.MaxTries;
max_success = def.MaxSuccess;
report = def.Verbosity;
...
if report
    ...
    [components, ~] = ReadNetlistFile('..\Altium\Sheet1.NET');
    PrikaziParticije(minimum(1, :), minimum(2, :), components);
end
